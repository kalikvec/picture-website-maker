...
import static com.drew.metadata.exif.ExifDirectory.*;

/**  */
public class ExifDescriptor extends TagDescriptor
{
  /**
   * Dictates whether rational values will be represented in decimal format in instances where decimal notation is elegant (such as 1/2 -> 0.5, but
   * not 1/3).
   */
  ...
  /**
   * Returns a descriptive value of the the specified tag for this image. Where possible, known values will be substituted here in place of the raw
   * tokens actually kept in the Exif segment. If no substitution is available, the value provided by getString(int) will be returned.
   *
  ...
  {
    if (!_directory.containsTag(TAG_THUMBNAIL_DATA))
    {
      return null;
    }

    int[] thumbnailBytes = _directory.getIntArray(TAG_THUMBNAIL_DATA);
  ...
  {
    if (!_directory.containsTag(TAG_ISO_EQUIVALENT))
    {
      return null;
    }

    int isoEquiv = _directory.getInt(TAG_ISO_EQUIVALENT);
  ...
  {
    if (!_directory.containsTag(TAG_REFERENCE_BLACK_WHITE))
    {
      return null;
    }
  ...
  {
    if (!_directory.containsTag(TAG_EXIF_VERSION))
    {
      return null;
    }

    int[] ints = _directory.getIntArray(TAG_EXIF_VERSION);

    return convertBytesToVersionString(ints);
  }

  private String getFlashPixVersionDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_FLASHPIX_VERSION))
    {
      return null;
    }

    int[] ints = _directory.getIntArray(TAG_FLASHPIX_VERSION);
  ...
  {
    if (!_directory.containsTag(TAG_SCENE_TYPE))
    {
      return null;
    }

    int sceneType = _directory.getInt(TAG_SCENE_TYPE);
  ...
  {
    if (!_directory.containsTag(TAG_FILE_SOURCE))
    {
      return null;
    }

    int fileSource = _directory.getInt(TAG_FILE_SOURCE);
  ...
  {
    if (!_directory.containsTag(TAG_EXPOSURE_BIAS))
    {
      return null;
    }

    Rational exposureBias = _directory.getRational(TAG_EXPOSURE_BIAS);
  ...
  {
    if (!_directory.containsTag(TAG_MAX_APERTURE))
    {
      return null;
    }

    double        apertureApex = _directory.getDouble(TAG_MAX_APERTURE);
  ...
  {
    if (!_directory.containsTag(TAG_APERTURE))
    {
      return null;
    }

    double        apertureApex = _directory.getDouble(TAG_APERTURE);
    double        rootTwo      = Math.sqrt(2);
    double        fStop        = Math.pow(rootTwo, apertureApex);
    DecimalFormat formatter    = new DecimalFormat("0.#");
  ...
  {
    if (!_directory.containsTag(TAG_EXPOSURE_PROGRAM))
    {
      return null;
    }
    ...
    switch (_directory.getInt(TAG_EXPOSURE_PROGRAM))
    {
      case 1:
      ...
      default:
        return "Unknown program (" + _directory.getInt(TAG_EXPOSURE_PROGRAM) + ")";
    }
  }

  private String getYCbCrSubsamplingDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_YCBCR_SUBSAMPLING))
    {
      return null;
    }
  ...
  {
    if (!_directory.containsTag(TAG_PLANAR_CONFIGURATION))
    {
      return null;
    }
    ...
    // plane format.
    switch (_directory.getInt(TAG_PLANAR_CONFIGURATION))
    {
      case 1:
  ...
  {
    if (!_directory.containsTag(TAG_SAMPLES_PER_PIXEL))
    {
      return null;
    }

    return _directory.getString(TAG_SAMPLES_PER_PIXEL) + " samples/pixel";
  }

  private String getRowsPerStripDescription()
  {
    if (!_directory.containsTag(TAG_ROWS_PER_STRIP))
    {
      return null;
    }

    return _directory.getString(TAG_ROWS_PER_STRIP) + " rows/strip";
  }

  private String getStripByteCountsDescription()
  {
    if (!_directory.containsTag(TAG_STRIP_BYTE_COUNTS))
    {
      return null;
    }

    return _directory.getString(TAG_STRIP_BYTE_COUNTS) + " bytes";
  }

  private String getPhotometricInterpretationDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_COMPRESSION))
    {
      return null;
    }
    ...
    switch (_directory.getInt(TAG_COMPRESSION))
    {
      case 1:
        return "Monochrome";
  ...
  {
    if (!_directory.containsTag(TAG_BITS_PER_SAMPLE))
    {
      return null;
    }

    return _directory.getString(TAG_BITS_PER_SAMPLE) + " bits/component/pixel";
  }

  private String getThumbnailImageWidthDescription()
  {
    if (!_directory.containsTag(TAG_THUMBNAIL_IMAGE_WIDTH))
    {
      return null;
    }

    return _directory.getString(TAG_THUMBNAIL_IMAGE_WIDTH) + " pixels";
  }

  private String getThumbnailImageHeightDescription()
  {
    if (!_directory.containsTag(TAG_THUMBNAIL_IMAGE_HEIGHT))
    {
      return null;
    }

    return _directory.getString(TAG_THUMBNAIL_IMAGE_HEIGHT) + " pixels";
  }

  private String getFocalPlaneXResolutionDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_FOCAL_PLANE_X_RES))
    {
      return null;
    }
  ...
  {
    if (!_directory.containsTag(TAG_FOCAL_PLANE_UNIT))
    {
      return null;
    }
    ...
    // '2' inch, '3' centimeter.
    switch (_directory.getInt(TAG_FOCAL_PLANE_UNIT))
    {
      case 1:
        return "(No unit)";
  ...
  {
    if (!_directory.containsTag(TAG_EXIF_IMAGE_WIDTH))
    {
      return null;
    }

    return _directory.getInt(TAG_EXIF_IMAGE_WIDTH) + " pixels";
  }

  private String getExifImageHeightDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_EXIF_IMAGE_HEIGHT))
    {
      return null;
    }

    return _directory.getInt(TAG_EXIF_IMAGE_HEIGHT) + " pixels";
  }

  private String getColorSpaceDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_COLOR_SPACE))
    {
      return null;
    }

    int colorSpace = _directory.getInt(TAG_COLOR_SPACE);
  ...
  {
    if (!_directory.containsTag(TAG_FOCAL_LENGTH))
    {
      return null;
    }

    DecimalFormat formatter   = new DecimalFormat("0.0##");
    Rational      focalLength = _directory.getRational(TAG_FOCAL_LENGTH);
  ...
  {
    if (!_directory.containsTag(TAG_FLASH))
    {
      return null;
    }
    ...
    switch (_directory.getInt(TAG_FLASH))
    {
      case 0:
      ...
      default:
        return "Unknown (" + _directory.getInt(TAG_FLASH) + ")";
    }
  }

  private String getWhiteBalanceDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_WHITE_BALANCE))
    {
      return null;
    }
    ...
    switch (_directory.getInt(TAG_WHITE_BALANCE))
    {
      case 0:
        return "Unknown";
      ...
      default:
        return "Unknown (" + _directory.getInt(TAG_WHITE_BALANCE) + ")";
    }
  }

  private String getMeteringModeDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_METERING_MODE))
    {
      return null;
    }
    ...
    int meteringMode = _directory.getInt(TAG_METERING_MODE);
  ...
  {
    if (!_directory.containsTag(TAG_SUBJECT_DISTANCE))
    {
      return null;
    }

    Rational      distance  = _directory.getRational(TAG_SUBJECT_DISTANCE);
    DecimalFormat formatter = new DecimalFormat("0.0##");
  ...
  {
    if (!_directory.containsTag(TAG_COMPRESSION_LEVEL))
    {
      return null;
    }

    Rational compressionRatio = _directory.getRational(TAG_COMPRESSION_LEVEL);
  ...
  {
    if (!_directory.containsTag(TAG_THUMBNAIL_LENGTH))
    {
      return null;
    }

    return _directory.getString(TAG_THUMBNAIL_LENGTH) + " bytes";
  }

  private String getThumbnailOffsetDescription()
  {
    if (!_directory.containsTag(TAG_THUMBNAIL_OFFSET))
    {
      return null;
    }

    return _directory.getString(TAG_THUMBNAIL_OFFSET) + " bytes";
  }

  private String getYResolutionDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_Y_RESOLUTION))
    {
      return null;
    }

    Rational inverseResolution = _directory.getRational(TAG_Y_RESOLUTION);

    return inverseResolution.getReciprocal().toSimpleString(_allowDecimalRepresentationOfRationals) + " " + getResolutionDescription().toLowerCase();
  }

  private String getXResolutionDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_X_RESOLUTION))
    {
      return null;
    }

    Rational inverseResolution = _directory.getRational(TAG_X_RESOLUTION);
  ...
  {
    if (!_directory.containsTag(TAG_EXPOSURE_TIME))
    {
      return null;
    }

    return _directory.getString(TAG_EXPOSURE_TIME) + " sec";
  }

  private String getShutterSpeedDescription() throws MetadataException
  {
    if (!_directory.containsTag(TAG_SHUTTER_SPEED))
    {
      return null;
    }

    // Incorrect math bug fixed by Hendrik Wördehoff - 20 Sep 2002
    int apexValue = _directory.getInt(TAG_SHUTTER_SPEED);
  ...
  {
    if (!_directory.containsTag(TAG_FNUMBER))
    {
      return null;
    }

    Rational      fNumber   = _directory.getRational(TAG_FNUMBER);
    DecimalFormat formatter = new DecimalFormat("0.#");
  ...
  {
    if (!_directory.containsTag(TAG_YCBCR_POSITIONING))
    {
      return null;
    }

    int yCbCrPosition = _directory.getInt(TAG_YCBCR_POSITIONING);
  ...
  {
    if (!_directory.containsTag(TAG_ORIENTATION))
    {
      return null;
    }

    int orientation = _directory.getInt(TAG_ORIENTATION);
  ...
  {
    if (!_directory.containsTag(TAG_RESOLUTION_UNIT))
    {
      return "";
    }

    // '1' means no-unit, '2' means inch, '3' means centimeter. Default value is '2'(inch)
    int resolutionUnit = _directory.getInt(TAG_RESOLUTION_UNIT);
  ...
  {
    if (!_directory.containsTag(TAG_SENSING_METHOD))
    {
      return null;
    }
    ...
    int sensingMethod = _directory.getInt(TAG_SENSING_METHOD);
  ...
  /**
   * Takes a series of 4 bytes from the specified offset, and converts these to a well-known version number, where possible. For example, (hex) 30 32
   * 31 30 == 2.10).
   *
