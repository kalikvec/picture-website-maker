...
import static com.drew.metadata.exif.ExifDirectory.TAG_THUMBNAIL_LENGTH;
import static com.drew.metadata.exif.ExifDirectory.TAG_THUMBNAIL_OFFSET;

/**
 * Extracts Exif data from a JPEG header segment, providing information about the camera/scanner/capture device (if available). Information is
 * encapsulated in an <code>Metadata</code> object.
 *
...
public class ExifReader implements MetadataReader
{
  ...
  /**
   * Represents the native byte ordering used in the JPEG segment. If true, then we're using Motorolla ordering (Big endian), else we're using Intel
  ...
  private void extractThumbnail(Directory exifDirectory)
  {
    ...
    if (!exifDirectory.containsTag(TAG_THUMBNAIL_LENGTH) || !exifDirectory.containsTag(TAG_THUMBNAIL_OFFSET))
    {
      return;
    }
    ...
    try
    {
      offset = exifDirectory.getInt(TAG_THUMBNAIL_OFFSET);
      length = exifDirectory.getInt(TAG_THUMBNAIL_LENGTH);
      ...
      exifDirectory.setByteArray(TAG_THUMBNAIL_DATA, result);
    }
    catch (Throwable e)
    {
   ...
   *                     _metadata.setFloat(DIRECTORY_EXIF_EXIF, TAG_FNUMBER, (float)Math.exp(convertTagToNumber(tagValueOffset, formatCode) *
   * Math.log(2) * 0.5));
   *                 }
   *                 break;
   ...
   *                     _metadata.setFloat(DIRECTORY_EXIF_EXIF, TAG_EXPOSURE_TIME, (float)(1 / Math.exp(convertTagToNumber(tagValueOffset,
   * formatCode) * Math.log(2))));
   *                 }
   *                 break;
   *         }
   *     }
  ...
  private int get16Bits(int offset)
  {
    ...
    {
      throw new ArrayIndexOutOfBoundsException("attempt to read data outside of exif segment (index " + offset + " where max index is "
                                                 + (_data.length - 1) + ")");
    }
  ...
  private int get32Bits(int offset)
  {
    ...
    {
      // Motorola big first
      return ((_data[offset] << 24) & 0xFF000000) | ((_data[offset + 1] << 16) & 0xFF0000) | ((_data[offset + 2] << 8) & 0xFF00)
               | (_data[offset + 3] & 0xFF);
    }
    else
    {
      // Intel ordering
      return ((_data[offset + 3] << 24) & 0xFF000000) | ((_data[offset + 2] << 16) & 0xFF0000) | ((_data[offset + 1] << 8) & 0xFF00)
               | (_data[offset] & 0xFF);
    }
  }
}
